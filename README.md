...
## 📊 Dataset Overview
**Source:** [CIC-DDoS 2019 (Kaggle)](https://www.kaggle.com/datasets/rodrigorosasilva/cic-ddos2019-30gb-full-dataset-csv-files)  
**Total Combined Rows:** 1,130,650  
**Final Processed Rows:** 423,722  
**Classification Types:** 12 Attack Classes + 1 Benign Class  
**BENIGN: 0
DNS: 1
LDAP: 2
MSSQL: 3
...
Features were automatically separated based on their unique value counts. Features with a low cardinality (e.g., `Protocol` with primarily 3 unique states, and TCP Flags) were routed to the categorical pipeline.
...
4. Standard 1D Convolutional Neural Network (1D CNN)
5. **1D CNN + Inception Module (🏆 Best Performing)**
...
### Metrics Table for different models
![Metrics_Table](images/metrictable.png)
...
```bash
git clone [https://github.com/vansh412f/NIDS_CNN.git](https://github.com/vansh412f/NIDS_CNN.git)
cd NIDS_CNN
```
...
### 3. Usage
Due to size constraints, the raw CIC-DDoS 2019 dataset is not included in this repository. Ensure you download the raw CSVs from Kaggle, then run the preprocessing scripts in order before initiating model training.
